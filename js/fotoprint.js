...
(function () {
  "use strict";
  const HM = window.HM;
  const id = new URLSearchParams(location.search).get("id") || HM.photos[0].id;
  const ph = HM.photos.find(x => x.id === id) || HM.photos[0];
  document.title = `${ph.title} — Hazel Media`;
  document.getElementById("photoProd").innerHTML = `
    <div class="big">
      <img src="${ph.img}" alt="${ph.title} — watermarked catalog preview">
      <div class="wm"></div>
    </div>
    <div class="pp-info">
      <span class="pno">FOTO ${ph.id}</span>
      <h1>${ph.title}</h1>
      <p class="desc">${ph.desc}</p>
      ...
      <a class="btn btn--solid" href="contact.html">Vraag naar deze foto <span class="arr">→</span></a>
    </div>`;
})();
